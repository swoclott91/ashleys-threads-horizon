...
- **UI:** **Cart / drawer** (`data-context="cart"`): condensed strip — status line (`cart_condensed_status`, e.g. “$X away from …”), horizontal milestone nodes (labels + theme `truck` icon for the current **shipping** step), tap opens a `<dialog>` “Bulk savings” sheet with vertical tier list (check burst / lock from `<template>` clones in `at-discount-progress.liquid`), disclaimer + **Got it**. **PDP / bulk-modal:** full-width bar unchanged — top “add more”, track + dots, bottom achievement + info popover.
