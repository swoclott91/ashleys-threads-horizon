...
- **Products panel:** `at_panel_item_handle` still opens category sidebar chrome; category grids JIT via `at-menu-data`. Brands sidebar item is skipped if still present in `at-categories`.
- **Column ranges:** `#-Ba | Be-C | D-H | I-L | M-O | P-S | T-Z` (`snippets/at-menu-brands-alpha-columns.liquid`). Digits → `#-Ba`; `B` before `BE` → `#-Ba`; `Be+` → `Be-C`.
- **Admin setup:** Ensure `main-menu` has a top-level **Brands** link (handle `brands`). Populate **`at-brands`** with every brand collection (titles below). Create **`at-featured-brands`** with ~6 logo brands (e.g. AllPro, Hanes, Harriton, Team 365, LEGACY, Bella + Canvas). Remove Brands from `at-categories` if it was nested under Products.
