...
| **Palette gotcha (this store)** | `color_palette.foreground` is a **gold `#bb987c`** and `page_text_color`/`--color-border` is **`#2c2c2c`** (neutral, NOT gold). Anything inheriting `--color-foreground` (headings, color names, prices, discount status) renders gold, so `assets/at-bulk-grid.css` **remaps `--color-foreground`/`--color-foreground-rgb` to `#2c2c2c`** on the three modal shells (`.at-bulk-grid-modal__inner`, `.at-buy-buttons__bulk-dialog-inner`, `.popup-link__inner:has(.at-bulk-grid)`) so regular text is non-gold. **Grid lines are intentionally gold**: `snippets/color-palette.liquid` emits **`--at-bulk-gold-rgb`** (= palette foreground rgb) on `:root`; `--at-bulk-line` (`/0.5`) + `--at-bulk-line-strong` (`/0.65`, qty-input + swatch borders) use it. The discount rail forces **`border-radius:999px`** for pill ends (theme `pills` radius is `0`). **Bulk status** (`.at-dp__cart-status`) is bold + heading-colored with no green goal, and left-aligned on desktop — scoped to the modal shells so PDP/cart strips keep their muted centered style. Rail colors/semantics: **green active = `--at-dp-accent` (`color5` `#243e34`) = what's actually in the cart (`committedBasis` = `#subtotal`)**; **gold pending = `--at-dp-pending` (default palette gold `#bb987c`) = the effect of previewed-but-not-yet-added items (`committedBasis` → `previewBasis`/projected)**; track = tinted empty-track. `#renderCartCondensed` computes both via a shared `basisToPct(basis)` interpolator (green = `basisToPct(committedBasis)`, gold end = `basisToPct(previewBasis)`). In `cart` context the two bases are equal so only green shows. |
...
- **UI:** **One layout everywhere** — condensed status line + **two-tone rail** + **milestone text labels UNDER the bar** (`.at-dp__cart-labels` > `.at-dp__cart-label`, positioned by `--at-dp-node-left`; first/last edge-align via `--first`/`--last`). **Rail = green `.at-dp__cart-line-active`** (continuous progress up to basis, interpolated within the current tier) **+ tan `.at-dp__cart-line-pending`** (green end → next milestone) over the track, with **faint dashed `.at-dp__cart-tick` marks** at interior milestones (`.at-dp__cart-ticks` overlay). Reached, non-shipping discount tiers are shown by **green + bold** styling only (`--reached:not(--shipping)` → accent color + bold) — NO “applied” text (it overlapped in tight strips; `node_applied` i18n is now unused). Shipping shows the tier name; all discount tiers show the short benefit label (e.g. “15%”); shipping + the next target stay neutral. **No more circular on-rail nodes** (`.at-dp__cart-node*` removed). Tap opens **Bulk savings** `<dialog>` (disclaimer + qualify link in footer; the dialog still uses truck/icon mounts). **`at-dp-condensed-strip--tight`** on drawer strip, bulk modals (`at-bulk-grid-modal`, `at-bulk-add-to-cart`, `at-popup-link`), **PDP** (`blocks/at-discount-progress`), and cart page strip (`main-cart` `.cart-page__at-dp-strip`). Drawer: **`header-actions`** header row **Cart | strip | close**.
...
- **Brands = standalone top-nav (directory mode):** Handle `at_brands_item_handle` (default `brands`) opens `snippets/at-menu-brands-directory.liquid` — alphabet text columns (`#-Ba` … `T-Z`) + right-rail featured logos. SSR'd (no `at-menu-data` fetch). Source: `at_brands_menu` linklist; featured: `at_featured_brands_menu` (≈4–6 collection links, `collection.image` only). CTAs: `at_brands_cta_*` settings.
- **Products nav modes (`at_products_nav_mode`):**
  - **`top_level` (near-term default):** No Products sidebar. Each `main-menu` item whose **handle matches** an `at-categories` category opens `snippets/at-menu-products-category-panel.liquid` (columns + featured only). Example top nav: T-shirts, Sweatshirts, Tank Tops, Polos, Hats, Activewear, Bags, Brands, Sale.
  - **`sidebar` (preserved):** Single `at_panel_item_handle` trigger opens `snippets/at-menu-products-panel.liquid` with left category rail. Switch back when Services+ need a denser IA.
- **Products panel content:** Linklist-first 3-level menu on `at_categories_menu`: **Category → column (Style/Fit/…) → leaf collection links**. Leaf links are **text-only** (no collection thumb boxes). Featured rail does the visual work. Flat panels use **`--page-margin`** + submenu block padding; column headings / leaves use **`--menu-parent-*` / `--menu-child-*`** from `submenu-font-styles` (`menu_font_style` on AT Menu). Sidebar mode: inactive categories defer featured image via `data-at-thumb-src`; JS hydrates on category activate. Featured card + icon from nested **`_at-menu-category`** blocks (match `category_handle`).
- **Products admin setup:** Keep `at-categories` as 3-level. For **top_level**, put categories on `main-menu` with handles matching `at-categories` (e.g. `t-shirts`, `sweatshirts-hoodies`). Add AT Menu category blocks (icon, featured image/collection, eyebrow/title/CTA). Missing icons show an empty slot until SVGs land.
- **Column ranges (Brands):** `#-Ba | Be-C | D-H | I-L | M-O | P-S | T-Z` (`snippets/at-menu-brands-alpha-columns.liquid`). Digits → `#-Ba`; `B` before `BE` → `#-Ba`; `Be+` → `Be-C`.
- **Admin setup (Brands):** Ensure `main-menu` has a top-level **Brands** link (handle `brands`). Populate **`at-brands`** with every brand collection. Create **`at-featured-brands`** with ~6 logo brands. Remove Brands from `at-categories` if nested under Products.
- **Brand list (Ashley's Threads, 2026-09):** 47 Brand, A4, Adams Headwear, AllMade, Alleson Athletic, AllPro, Alpine Fleece, Alternative Apparel, American Apparel, ANETIK, Anvil, Artisan Collection by Reprime, Atlantis Headwear, Augusta Sportswear, Authentic Pigment, Badger, BAGedge, Bayside, Bella + Canvas, Berne Apparel, Big Accessories, Boxercraft, Bulwark, Burnside, C2 Sport, Canvas for Good, CAP AMERICA, Carmel Towel Company, Chef Designs, Classic Caps, Code Five, Colortone, Comfort Colors, ComfortWash by Hanes, CORE365, CornerStone, Devon & Jones, Dickies, District, Doggie Skins, DRI DUCK, econscious, Ei Lo, Extreme, Flexfit, Fruit of the Loom, Gaiter King, Gildan, Hanes, Harriton, Helen Jon, Holloway, HUK, Imperial, Independent Trading Co., Infinity Her, J. America, Jaanuu, Jerzees, Kastlfel, Kati, Kishigo, Lane Seven, LAT, LEGACY, Liberty Bags, LOCALE, Los Angeles Apparel, M&O, Marmot, Mercer+Mettle, MERET, MV Sport, Nautica, Next Level Apparel, Nomadix, North End, OAD, Onna, Outdoor Cap, Paragon, Port & Company, Port Authority, Pukka, Puma Golf, Q-Tees, Rabbit Skins, RealTree, Recover, Red House, Red Kap, Richardson, Russell Athletic, Russell Outdoors, Shaka Wear, SoftShirts, Sport-Tek, Sportsman, Spyder, StarTee, SubliVie, TASC Performance Inc, Team 365, The Game, Threadfast Apparel, Top of the World, TriDri, Tultex, UltraClub, US Blanks, Valucap, Volunteer Knitwear, Weatherproof, Wink, YP Classics, Zero Restriction.
- **`at-menu-data`:** Mobile category drill-downs only (Products column body). Desktop Products/Brands are SSR'd.
...
- **Brand search listeners:** Prefer `capture: true` on the dropdown `ref="panel"` for `input` / clear `click` so filtering still runs reliably. Directory mode filters `.at-brands-dir__link` and hides empty columns.
...
- **Active top-nav indicator:** No caret on mega / dropdown triggers. Open items underline via `text-decoration` on `.menu-list__link-title` when the host has `[data-open]` or the trigger has `aria-expanded="true"` (do **not** use `::after` on `.menu-list__link` — Horizon uses that for the mega-menu hover bridge). Tighter `padding-inline` keeps category labels closer.
- **Custom theme icons:** Adding a picker option requires all of: **`snippets/icon.liquid`** (`when` case + paths), **`snippets/icon-or-image.liquid`** (non-`0 0 20 20` viewBox if paths exceed 20×20), **`blocks/icon.liquid`** and **`blocks/_accordion-row.liquid`** schema options, and **`locales/en.default.schema.json`** → `options.fast_truck` (use `t:options.<key>` with no spaces). Edits under **`.shopify-remote-main/`** are not deployed — change the repo root files. Category mega-menu icons only need the `icon.liquid` `when` case (name referenced from AT Menu category blocks).
- **AT nav + `menu-list__link`:** In the header block, links use both classes. **`blocks/_header-menu.liquid`** sets `.menu-list__link { flex-direction: column }` for the mega-menu bridge; override with **`.menu-list__link.at-menu__nav-link { flex-direction: row }`** in `at-menu.css`.
