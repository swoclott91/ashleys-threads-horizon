...
- **Products panel content:** Linklist-first 3-level menu on `at_categories_menu`: **Category → column (Style/Fit/…) → leaf collection links**. Leaf thumbs = `collection.image` / `featured_image` only (never product walk). Sidebar mode: inactive categories defer thumbs via `data-at-thumb-src`; JS hydrates on category activate. Featured card + icon from nested **`_at-menu-category`** blocks (match `category_handle`).
