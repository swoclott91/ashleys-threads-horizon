...
- **Products panel:** `at_panel_item_handle` opens `snippets/at-menu-products-panel.liquid`. Linklist-first 3-level menu on `at_categories_menu`: **Category → column (Style/Fit/…) → leaf collection links**. Leaf thumbs = `collection.image` / `featured_image` only (never product walk). Inactive categories defer thumbs via `data-at-thumb-src`; JS hydrates on category activate. Featured card + icon from nested **`_at-menu-category`** blocks (match `category_handle`).
- **Products admin setup:** Rebuild `at-categories` as 3-level. Add AT Menu category blocks (icon name from `icon.liquid`, featured image/collection, eyebrow/title/CTA). Missing icons show an empty slot until SVGs land.
