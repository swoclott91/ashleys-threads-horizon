...
| `blocks/size-guide.liquid` | PDP-only size guide from **`custom.size_guide`** JSON metafield: **Standalone** (`dialog-component` + trigger, reuses popup-link dialog classes and `settings.popover_color_scheme`) or **Content only** for nesting inside **Popup link**. Snippets: `size-guide-modal.liquid`, `size-guide-section.liquid`, **`size-guide-sanitize-text.liquid`** (strips vendor names from displayed JSON/metafield strings). |
