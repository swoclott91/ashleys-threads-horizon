...
- **UI:** **Cart / drawer** (`data-context="cart"`): **Drawer header row** — **Cart + bubble | condensed strip | close** (`snippets/header-actions.liquid`). **Bulk modals** (`data-context="bulk-modal"`): **same condensed** UI (status + rail + nodes, expand opens **Bulk savings** `<dialog>`). Scheme wrap includes **`at-dp-condensed-strip--tight`** in `snippets/at-bulk-grid-modal.liquid`, `blocks/at-buy-buttons.liquid`, and `blocks/at-popup-link.liquid` so styles align with the cart drawer tight strip (`:is(..., .at-dp-condensed-strip--tight)` in `assets/at-discount-progress.css`). **PDP** (`data-context="product"`): **full** progress bar + info popover (unchanged).
- **Cart drawer “empty” styling:** Do not use a dialog class like `cart-drawer--empty` that is only set in Liquid at first paint. AJAX cart updates use **hydration** morph on `[data-hydration-key="cart-drawer-inner"]` only; the `<dialog>` does not re-render, so an “empty” class on it stayed wrong after add-to-cart and **empty-state flex centering** (`height: 100dvh`, etc.) could hide the drawer header and condensed discount bar after close/reopen. Tie empty-only rules to real inner DOM, e.g. **`.cart-drawer__dialog:has(.cart-drawer__heading--empty)`** (see `snippets/header-actions.liquid`).
...
- **Next milestone visibility:** Full PDP bar uses `.at-dp__dot--next` — avoid `color-mix` with variant-hover + background (often near-white on white). Use **input background fill + `border-color: var(--color-selected-variant-background)`** like the condensed hollow node. Condensed **current** node rules live under **`.at-dp--cart-condensed`** only (`assets/at-discount-progress.css`).
