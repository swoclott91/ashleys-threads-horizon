...
- **UI:** **Cart / drawer** (`data-context="cart"`): condensed strip — status + rail often **one horizontal row** (drawer header / cart page strip). Completed **discount** milestones use **`add-to-cart__added-icon` checkmark burst** in the dot; **shipping** milestones always use **truck**. Tap opens `<dialog>` “Bulk savings”. **PDP / bulk-modal:** full bar unchanged.
