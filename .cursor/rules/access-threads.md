...
| **Palette gotcha (this store)** | `color_palette.foreground` is a **gold `#bb987c`**, so anything inheriting `--color-foreground` (headings, color names, prices, discount status, table borders) renders gold. `assets/at-bulk-grid.css` **remaps `--color-foreground`/`--color-foreground-rgb` to the neutral text color `#2c2c2c`** on the three modal shells (`.at-bulk-grid-modal__inner`, `.at-buy-buttons__bulk-dialog-inner`, `.popup-link__inner:has(.at-bulk-grid)`) so regular text is non-gold. Grid/accordion separators use a light neutral **`--at-bulk-line`** (`rgb(var(--color-foreground-rgb)/0.12)`) instead of the gold `--color-border`; qty-input + swatch borders use `/0.25`. The discount rail forces **`border-radius:999px`** for pill ends (theme `pills` radius is `0`). |
