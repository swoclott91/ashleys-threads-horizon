...
- **AT mega panel `top`:** For triggers inside **`#header-component`**, use **`#header-component.getBoundingClientRect().bottom`** (`assets/at-menu.js`). That rect already accounts for the announcement bar (sibling above pushes `rect.top` down) and the full header height (e.g. ~66px component + ~43px bar ⇒ ~109px seam). Do **not** `Math.min` with **`.at-menu__nav.bottom`** — the nav is shorter than the header box and incorrectly pins ~92px. When the header sticks at the viewport top, the same call yields ~66px. Else **`Math.min(nav, row, trigger)`** fallback. Re-run **`#updatePanelTop`** in **`queueMicrotask`** after the double `rAF` on open. Seam overlap **`max(2, 1 + borderBottomWidth)`** on the relevant row. Dropdown **`z-index: calc(var(--layer-header-menu) + 1)`**.
