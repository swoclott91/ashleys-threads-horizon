...
- **AT mega panel `top`:** Menu row can be `.header__row--bottom` (`menu_row`). **Home vs inner pages:** link `getBoundingClientRect()` can read much lower than the real first-row seam (e.g. ~92px vs ~66px). When the trigger is in **`.header__row--top`**, use **`Math.min(#header-component.top + --top-row-height, .header__row--top.bottom, .at-menu__nav.bottom)`** — the nav strip is often shorter than the full row (logo column), so including **`.at-menu__nav.bottom`** avoids a panel `top` that tracks the row bottom (~announcement + tall row) instead of the links. **`--top-row-height`** is set on `#header-component` in `utilities.js` / inherited from `body` (`theme.liquid`). **`.header__row--bottom`:** **`Math.min(row.bottom, .at-menu__nav.bottom)`**. Else **`Math.min(nav, row, trigger)`** fallback. Re-run **`#updatePanelTop`** in **`queueMicrotask`** after the double `rAF` on open so `--top-row-height` is current. Seam overlap **`max(2, 1 + borderBottomWidth)`** on the relevant row. Dropdown **`z-index: calc(var(--layer-header-menu) + 1)`**.
