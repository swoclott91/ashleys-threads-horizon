...
- **UI:** **Cart / drawer** (`data-context="cart"`): condensed strip — status line (`cart_condensed_status`), horizontal milestones: **percent tiers use `benefitLabel` text; the free-shipping step (`kind: "shipping"` in milestones JSON) always uses the theme `truck` icon** (never “Free ship” text in the nodes). Strip is **compact** (minimal padding, no card border in drawer / cart page header context) and placed **next to the cart heading** (not above checkout totals). Tap opens `<dialog>` “Bulk savings” with vertical tier list + **Got it**. **PDP / bulk-modal:** full bar unchanged — top “add more”, track + dots, bottom achievement + info popover.
